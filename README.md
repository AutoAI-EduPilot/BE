...
공개 계정 역할은 **학습자(LEARNER)**와 **강사(INSTRUCTOR)**입니다. 두 역할 모두 개인 PDF 학습을 사용할 수 있고, 강사는 본인 강의실을 관리하거나 다른 강의실의 승인 멤버로서 공개 주차 자료를 조회·학습합니다(DEC-030). 자료와 사용자를 관리하는 **관리자(ADMIN)**는 MVP의 보조 사용자입니다.

## 2. MVP 기능

- 역할을 선택하는 회원가입, 로그인, 내 정보 조회, 회원 탈퇴
- PDF 학습 자료 업로드, 조회 및 삭제
- 강의실 개설·초대 참여·주차 자료·즉시 공지·캘린더 파생 조회
...
MVP에서 제외하거나 후순위로 두는 기능은 실시간 화상 강의, 강사-학습자 실시간 채팅, 결제, 복잡한 관리자 통계, 모든 LLM 출력의 완전 자동 사실 검증입니다. 강의실 최소셋은 DEC-030에 따라 포함하지만 Course, Lecture, Assignment, Notification, 통계·리마인더·예약 게시·CUSTOM 일정은 제외합니다.
...
EDUPILOT_AI_STREAM_IDLE_TIMEOUT
EDUPILOT_AI_GRADE_READ_TIMEOUT
EDUPILOT_ADMIN_INFRA_ENABLED
EDUPILOT_ADMIN_INFRA_REGION
EDUPILOT_ADMIN_INFRA_INSTANCES_PROD
EDUPILOT_ADMIN_INFRA_INSTANCES_DEV
EDUPILOT_ADMIN_INFRA_METRICS_CACHE_TTL
EDUPILOT_ADMIN_INFRA_COST_CACHE_TTL
EDUPILOT_QUIZ_PASS_RATIO
EDUPILOT_QUIZ_PROPOSAL_MIN_PAGE_TEXT_LENGTH
